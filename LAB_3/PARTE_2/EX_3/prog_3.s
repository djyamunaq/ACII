...
main:       
            li      $s0, SFR_BASE_HI            
            # RB1 INPUT
            lw      $t2, TRISB($s0)             # READ (MemAddr = 0xBF880000 + 0x6040)
            ori     $t2, $t2, 0x0002            # MODIFY (bit1=1)
            sw      $t2, TRISE($s0)             # WRITE (TRISB1 = 1 => INPUT)
            # RE0 OUTPUT
            ...
            # RE1 OUTPUT
            lw      $t2, TRISE($s0)             # READ (MemAddr = 0xBF880000 + 0x6100)
            andi    $t2, $t2, 0xFFFD            # MODIFY (bit1=0)
            ...
            lw      $t2, PORTB($s0)             # READ (MemAddr = 0xBF880000 + 0x6100)
            andi    $s3, $t2, 0x0002            # GET (bit1) => lastState
...
if2:         
            beq     $t2, $s3, endif2            # if(currentState = lastState) goto endIf 
            beq     $t2, 0, else2                
            li      $s2, 1
            li      $s1, 0
            move    $s3, $t2
